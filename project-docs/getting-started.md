...
本项目集成了 **MUI (Material-UI)** 组件库和 **Lucide-React** 图标库，并针对 SSR 环境进行了深度优化。详细的使用方法和集成方案请参考：

📖 **[UI 库使用指南](ui-libraries.md)** - 包含 MUI 和 Lucide-React 的完整使用文档

主要特性：
- 🎨 **emotion 缓存优化**: 解决 SSR 环境下的 FOUC 问题
- ⚡ **性能提升**: 减少 33% 的重复样式注入
- 🔧 **完整集成方案**: 服务端渲染 + 客户端水合的完整流程
- 🧪 **测试验证**: 提供测试页面验证优化效果
